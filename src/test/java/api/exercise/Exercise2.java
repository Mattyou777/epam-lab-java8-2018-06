...
public class Exercise2 {
  ...
  /**
   * Выполняет операцию сканирования в однопоточном режиме. Не модифицирует исходный набор данных.
   *
  ...
  private static <T> T[] sequentialPrefix(T[] source, BinaryOperator<T> operator) {
    ...
    for (int i = 0; i < log2(n) + 1; i++) {
      ...
      for (int j = n - 1; j > p - 1; j--) {
        result[j] = operator.apply(result[j], result[j - p]);
      }
    }
    return result;
  }
  ...
  private static int pow(int base, int degree) throws IllegalArgumentException {
    ...
    return (int) Math.pow((int) base, (int) degree);
  }
}
