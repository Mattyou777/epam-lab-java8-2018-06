...
public class Exercise2 {
  ...
  private static boolean getPersonHasEmptyFirstNamePredicate(Person person) {
    return Strings.isNullOrEmpty(person.getFirstName());
  }

  private static boolean getPersonHasEmptyLastNamePredicate(Person person) {
    return Strings.isNullOrEmpty(person.getLastName());
  }

  @Test
  public void personHasNotEmptyLastNameAndFirstName() {
    Predicate<Person> validate = person -> !getPersonHasEmptyFirstNamePredicate(person)
        && !getPersonHasEmptyLastNamePredicate(person);
  ...
  public void personHasNotEmptyLastNameAndFirstNameUsingLogicalOperators() {
    Predicate<Person> personHasEmptyFirstName = Exercise2::getPersonHasEmptyFirstNamePredicate;
    Predicate<Person> personHasEmptyLastName = Exercise2::getPersonHasEmptyLastNamePredicate;
